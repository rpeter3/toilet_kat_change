...
const char *SOFTWARE_VERSION = "0.1.3";
...
// True when OTA transfer is in progress - do not expire window or shut down BLE
...
void mcp_setup()
{
    // MCP I2C always matches toilet_kat_change: GPIO6 SDA, GPIO7 SCL
    ...
    Serial.println("MCP23017 Initialized Successfully.");
    // Setup button pins on expander (button1 is on main board GPIO2/controlPanelWake)
...
// Function to write a value to a specific MCP23017 pin (always runs so LEDs/buttons work in dev mode)
void mcp_digitalWrite(int pin, int value)
{
    mcp.digitalWrite(pin, value);
}

// Function to read from a specific MCP23017 pin (always runs so buttons work in dev mode)
...
void setup()
{
    ...
    delay(4000);          // Longer delay to ensure Serial is 
    ...
    // LED startup test: all on, then all off
    ...
    // Always set button 1 pin so it doesn't float (avoids false "held" and cut mode animation in dev)
    ...
    // Test heater current detection (skip in dev mode)
    if (!dev)
        testHeaterCurrent();

    motors.enableDrivers();
    motors3.enableDrivers(); // Enable M3 motor shield
    // Locate motor position (skip in dev mode)
    if (!dev)
        locateMotorPos();
...
void loop()
{
    ...
    if (otaEnabled)
    {
        ...
        // Check if OTA window has expired (1 minute) - never expire while transfer in progress
        unsigned long currentMillis = millis();
        if (!otaTransferInProgress() && (currentMillis - otaWindowStartTime >= OTA_WINDOW_DURATION))
        {
    ...
    if (bleEnabled && !otaTransferInProgress() && (millis() - bleStartupTime > BLE_TIMEOUT))
    {
    ...
    if (digitalRead(controlPanelWake) == LOW)
...
const char *otaStateToString(OTAState state)
{
    ...
    case OTA_IDLE:
        return "OTA_IDLE";
    case OTA_PREPARING:
        return "OTA_PREPARING";
    case OTA_RECEIVING:
        return "OTA_RECEIVING";
    case OTA_VALIDATING:
        return "OTA_VALIDATING";
    case OTA_FINALIZING:
        return "OTA_FINALIZING";
    case OTA_ERROR:
        return "OTA_ERROR";
    case OTA_ROLLBACK:
        return "OTA_ROLLBACK";
