...
// TEST_MECHANISM_MOTOR: stress-test interrupt and close counter
volatile bool testInterrupted = false;
uint32_t mechanismCloseCount = 0;
...
void setup() {
  ...
  //testHeaterCurrent(); //igore for test loop
  ...
  // TEST_MECHANISM_MOTOR: Auto-start stress test on boot (press Feed button to stop)
  SerialBLE_println("TEST_MECHANISM_MOTOR: Starting stress test - press Feed button to interrupt");
  isFlushing = true;
}
...
void loop() {
  ...
  // TEST_MECHANISM_MOTOR: BLE timeout disabled - device stays on for stress test monitoring
#if 0
  ...
  if (!batteryDisplayMode) {
    // TEST_MECHANISM_MOTOR: Button 1 (Flush) restarts stress test if not running
    if (button1Pressed && !button1WasPressed && !isFlushing) {
      SerialBLE_println("TEST_MECHANISM_MOTOR: Button 1 - (re)starting stress test");
      isFlushing = true;
  ...
  // TEST_MECHANISM_MOTOR: Feed button (Button 2) interrupts the test
  if (isFlushing && (mcp_digitalRead(button2Pin) == LOW)) {
...
// TEST_MECHANISM_MOTOR: stress-test state machine (replaces full flush sequence)
enum TestStep { TEST_CLOSE_M1, TEST_PAUSE_5, TEST_STRAIN_M1, TEST_WAIT_10, TEST_REVERSE_M1, TEST_PAUSE_3 };
static TestStep testStep = TEST_CLOSE_M1;
static unsigned long testStepStart = 0;
static bool testM1Closing = false;
static bool testM1Reversing = false;
static bool testStrainStarted = false;
static bool testReverseExtraPhase = false;  // true = close switch opened, running 2s more (horizontal dead time)
...
void flushSequence() {
  if (testInterrupted) {
    ...
    testStep = TEST_CLOSE_M1;
    testM1Closing = false;
    testM1Reversing = false;
    testStrainStarted = false;
    testReverseExtraPhase = false;
    SerialBLE_println("TEST_MECHANISM_MOTOR: Interrupted by feed button");
    return;
  }
  ...
  switch (testStep) {
    case TEST_CLOSE_M1: {
      if (!testM1Closing) {
        SerialBLE_println("TEST: Close M1 until switch + stall current");
      ...
      if (digitalRead(microswitchClosePin) == LOW) {
        ...
        if (m1Current > 0.5f) {
          ...
          testM1Closing = false;
          mechanismCloseCount++;
          SerialBLE_println("MECH_CLOSE_COUNT=" + String(mechanismCloseCount));
          testStep = TEST_PAUSE_5;
          testStepStart = now;
        }
      }
      break;
    ...
    case TEST_PAUSE_5:
      if (now - testStepStart >= 5000) {
        testStep = TEST_STRAIN_M1;
        testStepStart = now;
        testStrainStarted = false;
      }
      break;
    ...
    case TEST_STRAIN_M1:
      if (!testStrainStarted) {
        SerialBLE_println("TEST: Strain M1 for 0.5s");
        motors.setM1Speed(400);
        testStrainStarted = true;
        testStepStart = now;
        return;
      }
      if (now - testStepStart >= (unsigned long)(MOTOR_CUT_TIME * 1000)) {
        motors.setM1Speed(0);
        testStep = TEST_WAIT_10;
        testStepStart = now;
      }
      break;

    case TEST_WAIT_10:
      if (now - testStepStart >= 10000) {
        testStep = TEST_REVERSE_M1;
        testM1Reversing = false;
      }
      break;

    case TEST_REVERSE_M1: {
      if (!testM1Reversing) {
        SerialBLE_println("TEST: Reverse M1 until close switch opens, then 2s more");
        motors.setM1Speed(-400);
        mechanismMotorRunning = true;
        testM1Reversing = true;
        testReverseExtraPhase = false;
        return;
      }
      if (!testReverseExtraPhase) {
        // Wait for close switch to read HIGH (mechanism no longer at closed position)
        if (digitalRead(microswitchClosePin) == HIGH) {
          testReverseExtraPhase = true;
          testStepStart = now;
        }
      } else {
        // Horizontal travel dead time: continue reversing for 2 more seconds
        if (now - testStepStart >= 2000) {
          motors.setM1Speed(0);
          mechanismMotorRunning = false;
          testM1Reversing = false;
          testReverseExtraPhase = false;
          testStep = TEST_PAUSE_3;
          testStepStart = now;
        }
      }
      break;
