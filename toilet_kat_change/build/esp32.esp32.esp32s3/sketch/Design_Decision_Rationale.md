#line 1 "C:\\Users\\richa\\Documents\\GitHub\\toilet_kat_change\\toilet_kat_change\\Design_Decision_Rationale.md"
...
- **Usable battery assessment** (`assessBatteryUsable`): user-facing battery % (LED bar, `GET_BATTERY`, flush block) is derived from a scaled heater load pulse, not idle open-circuit VMON. Assessment steps through duties up to `getHeaterPwmCapForVoltage(vIdle)`, records worst loaded voltage (and sag for diagnostics only), and maps usable % from `minLoadedBatteryV` (0%, default 10.0 V) to `usableVFull` (100%, default 11.4 V). Pass/fail is based on loaded voltage ≥ `minLoadedBatteryV` only — delta sag (`maxBatterySagV`) is logged but does not gate. Packs above 11.4 V under load report 100% (full plateau / ageing buffer). Soft margin for brownout is the 10.0 V floor vs ~9.5 V hardware brownout, not a large % threshold.
...
- **Runtime low-voltage debounce** (`checkRuntimeSagDebounced`): requires `RUNTIME_SAG_STRIKES` consecutive VMON samples below `minLoadedBatteryV` before latching low battery mid-heat (absolute floor, not delta sag).
